...
node* insertSequentially(node* root, int val){
    ...
    if(!iter){
        ...
        temp -> next = iter -> next;
        iter -> next = temp;
        printf("%d has been added into list", val);
...
node* deleteVal(node* root, int val){
    ...
    if(!iter){
        ...
        iter = iter -> next;
        free(temp);
        printf("%d is deleted from the head of list", val);
        return iter;
    }
    else{
        while((iter -> next) && (iter -> next -> val != val))
            iter = iter -> next;
        if(!(iter -> next))
            printf("The value of %d did not be found in list\n", val);
        else{
            temp=iter -> next;
            iter -> next = iter -> next -> next;
            free(temp);
            printf("%d is deleted from the list", val);
        }
        return root;
    }
}
